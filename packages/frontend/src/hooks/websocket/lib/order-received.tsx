...
export const orderReceivedEvent = (router: AppRouterInstance, client: QueryClient, data: WebsocketOrder) => {
	...
	client.setQueryData(["orders", "pending"], (prev: QueryPayload): QueryPayload => {
		...
		const description =
			productsLen == 0 ? (
				<p className="font-medium text-primary/60">
					<span className="font-semibold text-primary/80">{productTitle}</span> вартістю {orderPrice}
				</p>
			) : (
				<p className="font-medium text-primary/60">
					<span className="font-semibold text-primary/80">{productTitle}</span> та ще {count} вартістю {orderPrice}
				</p>
			);
		...
		return {
			...
			pages: prev.pages.map((page, index) =>
				index == 0 ? { ...page, count: page.count + 1, results: [data.order, ...page.results] } : page
			),
		};
	});
};
